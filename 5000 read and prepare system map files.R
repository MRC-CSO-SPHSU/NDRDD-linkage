...
ggsave(paste0(wd,"/Figure 1 CoProducedCommunityDectection.pdf"),
              device = "pdf", width = 13.9, height = 10.3, units = "in")

ggsave(paste0(wd,"/Figure 1 CoProducedCommunityDectection.jpeg"),
...
V(tbl_graph_raw)$x_coord <- igraph::degree(tbl_graph_raw, mode = "in")
V(tbl_graph_raw)$y_coord <- igraph::degree(tbl_graph_raw, mode = "out")

co_ordinates <- cbind(V(tbl_graph_raw)$x_coord , V(tbl_graph_raw)$y_coord )
co_ordinates <- as.data.frame(co_ordinates)
names(co_ordinates) <- c("x","y")

table(co_ordinates$x , co_ordinates$y)

###########In progress

equidistant_points_on_circle <- function(X, R) {
  # Calculate the angle between each point
  delta_theta <- 2 * pi / X
  
  # Initialize vectors to store x and y co_ordinates
  x_coordinates <- numeric(X)
  y_coordinates <- numeric(X)
  
  # Calculate the coordinates of each point on the circle
  for (i in 1:X) {
    theta <- (i - 1) * delta_theta
    x_coordinates[i] <- R * cos(theta)
    y_coordinates[i] <- R * sin(theta)
  }
  
  # Return a data frame with x and y coordinates
  return(data.frame(x = x_coordinates, y = y_coordinates))
}

# Example usage:
X <- 10  # Number of equidistant points
R <- 5   # Radius of the circle

equidistant_points <- equidistant_points_on_circle(X, R)
print(equidistant_points)

plot(equidistant_points)
...
for (i in co_ordinates$x) {
  for (j in co_ordinates$y) {
    ...
    if (count > 0) {
      ...
      points$x <- points$x + i  # Adjust x coordinates
      points$y <- points$y + j  # Adjust y coordinates
      new_coordinates <- rbind(new_coordinates, points)
    }
  }
}
...
ggraph(tbl_graph_raw , layout = cbind(V(tbl_graph_raw)$x_coord,V(tbl_graph_raw)$y_coord)) +
  ...
  geom_edge_link(arrow = arrow(length = unit(2, 'mm')), 
                 end_cap = circle(12, 'mm'),
                 color = "grey80") +
  geom_node_point(aes(size = V(tbl_graph_raw)$size), color = "white") +
  geom_node_point(aes(size = V(tbl_graph_raw)$size), 
                  color = colrs_vertex[V(tbl_graph_raw)$community], 
                  alpha = 0.5, shape = 20) +
  geom_node_text(aes(label = V(tbl_graph_raw)$name),
                 repel = TRUE, max.overlaps = Inf, 
                 color = "black", size = 2) +
  scale_size_continuous(range = c(2, 10)) +
  scale_x_continuous(breaks = seq(min(new_coordinates$x), max(new_coordinates$x), by = 1)) +
